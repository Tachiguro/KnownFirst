...
2. The feature is gated by `EnhancedTermRecognitionEnabled` (default OFF on `master`; a candidate branch in progress — Settings GUI & Learning-Day Defaults — Slice 2A, branch `feature/settings-gui-learning-day-defaults-v1`, initial commit `0d0d5b9f52a7823cb793dea94ca319f48fae9ec7` — changes the missing-preference default to ON without altering this gating mechanism itself; an explicitly persisted `false` always remains OFF regardless of the default. See [docs/CURRENT_WORK.md](CURRENT_WORK.md) "Settings GUI & Learning-Day Defaults — Slice 2A (Candidate in Progress)").
...
- **Active Review Words derivation-source indication (German Enhanced Term Recognition Package 5B):** the active Review Words page (distinct from the Preparation-time "review-words display path" fallback described above) may display a minimal, always-visible derivation-source notice for a candidate whose `ReviewCandidateDetails.Provenance` is `CandidateProvenanceKind.DerivedFromCompound` and whose `DerivationEvidence` is non-empty. This is display-only: it reads the already-existing `Provenance`/`DerivationEvidence` projection, never creates or fabricates a `WordOccurrenceEntity`, and does not change the active Review Words `Contexts` selection or count described above. The visible source is `DerivedTermEvidence.SourceSurfaceForm` only; it never exposes `SourceIdentity`, source positions, or `ComponentForm`. It does not alter decomposition identity, Known-suppression semantics, or the post-review-completion retention/context-recovery lifecycle described in this subsection, which remains the sole mechanism for recovering a derived candidate's source context after review completion. Normal Preparation Accept→Learning behavior remains provenance-agnostic: nothing downstream of Accept branches on `CandidateProvenanceKind`, and MarkKnown/Exclude cleanup semantics are unchanged.
