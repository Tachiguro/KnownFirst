...
- Implemented schema-neutral Wikipedia fallback orchestration in `LexicalEnrichmentService`. If the primary Wiktionary lookup returns a clean NotFound, the system gracefully queries Wikipedia in a separate lookup context, preserving essential redirect metadata without introducing provider cycles or requiring schema migrations.
