...
| 16 | Automated GUI validation | Current | P16-A — Android rendered-navigation foundation remains a non-binding candidate. After the renewed review found two MAJORs, final correction `4545fcc927ffdd787c548b447cbd21d087460e25` corrected early GUI-test MSBuild isolation and Release Notes screenshot-byte/hash identity. Final bounded validation is MSTest `160/0/0` and Node `7/0/0/0/0`; deterministic MSBuild property evaluation verified early isolation. PR-body and tracked-document reconciliation are complete; renewed independent review of the final remote PR state is pending. Manual merge remains user-only and approval-gated; `POST_MERGE_SYNC_ONLY` follows only a verified manual merge. Future rendered Android runtime validation remains pending; no matrix row is automated. Android-first deterministic automation precedes later standard Windows launcher integration. |
...
**Portable merge integrity hardening (Priority 15) is complete and binding `master` behavior.** PR #89 merged the Occurrence action-key correction (implementation `edbb49a87ff3f37337c413111a60f6cfa6805b88`, feature head `d45a7e8fad533ddda5dda425356bf2095e8bafb6`, merge commit `49d25cb8d7d113d1f0b1826369d9105a37d9207b`) after focused TDD **RED 0/2/0 → GREEN 2/0/0**, bounded `TEST_ONLY` **257/0/0**, independent review **0 BLOCKER / 0 MAJOR / 0 MINOR / 0 NIT**, and `POST_MERGE_SYNC_ONLY`. The legacy LearningReview label is not a current production-writer defect on the V1-to-V2 route. Priority 16 is Current through the active non-binding P16-A candidate; see [CURRENT_WORK.md](CURRENT_WORK.md) for lifecycle state.

### Priority-16 package sequence

- **P16-A:** Android automation foundation plus one pre-matrix scenario; the first review (`0/2/3/0`) and renewed review (`0/2/0/0`) required corrections. Final correction `4545fcc927ffdd787c548b447cbd21d087460e25` is pushed to PR #91, correcting early GUI-test intermediate/project-extension isolation and exact persisted Release Notes screenshot-byte/hash identity. Final bounded source-level validation is MSTest `160/0/0`, Node `7/0/0/0/0`, and deterministic MSBuild property evaluation; PR-body and tracked-document reconciliation are complete. Renewed independent review of the final remote PR state remains pending; manual merge is user-only and approval-gated, followed by `POST_MERGE_SYNC_ONLY` only after verified manual merge. Rendered Android runtime validation remains pending; no GUI matrix row is automated.
...
16. **Occurrence action-key correction:** merged and binding `master` behavior through PR #89 (implementation `edbb49a87ff3f37337c413111a60f6cfa6805b88`, feature head `d45a7e8fad533ddda5dda425356bf2095e8bafb6`, merge commit `49d25cb8d7d113d1f0b1826369d9105a37d9207b`; `POST_MERGE_SYNC_ONLY` complete). It uses the shared lookup-only `SourceMaterialArchiveId:Occurrence.Order` key rather than colliding `SentenceId:VocabularyId`; semantic occurrence identity and compatibility contracts are unchanged. Focused TDD **0/2/0 → 2/0/0**; bounded TEST_ONLY **257/0/0**; independent review **0 BLOCKER / 0 MAJOR / 0 MINOR / 0 NIT**. Priority 15 is Committed; Priority 16 is Current through active non-binding P16-A.
