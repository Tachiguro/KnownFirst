...
- **Safety contract:** dedicated package only, `com.tachiguro.knownfirst.guitest`; fail closed for every other KnownFirst package ID; already-installed application only; no build, package, install, update, uninstall, reset, or `pm clear`; no direct ADB command in the repository runner; `noReset=true`; `fullReset=false`; explicit Chromedriver executable required with no automatic acquisition; no automatic npm/dependency installation/update; loopback Appium server; fresh isolated application-private GUI-test profile; deterministic offline provider path. Appium readiness is fail-closed around the owned listener/PID: a free port is represented by an empty filtered listener set and is allowed normally, while an existing listener/collision or genuine listener-inspection failure aborts. The runner terminates only the stored process it launched. Before scenario success, runtime-observed rendered full commit, dirty state, package, and configuration must agree with the expected clean `.guitest` identity. GUI-test intermediate/project-extension state is isolated early through repository-root `Directory.Build.props` only for the opt-in Debug app project, leaving ordinary Debug and Core paths unaffected. Run-directory writes are restricted to a canonical strict descendant of `artifacts/gui-tests/android/runs`, exact dependency pins are contract-tested, and the screenshot summary SHA-256 is computed from the exact Release Notes PNG bytes persisted as the artifact.
...
- **Current evidence boundary:** source/configuration contracts, profile-isolation unit behavior, runner/evidence pure logic, static scenario/safety structure, and deterministic MSBuild property evaluation are validated. Property evaluation is source/build-configuration evidence only, not an Android platform build. The harness has **not** been runtime-executed; it does not prove Android platform build, APK/AAB creation, installation, Appium or UiAutomator2 runtime, Chromedriver/WebView compatibility, rendered interaction, actual screenshot correctness/runtime evidence, ADB/device/emulator behavior, matrix coverage, S36 automation, CI success, or release readiness. `MANUAL_ANDROID_GUI` remains a distinct scope and P16-A does not replace it.
- **Historical initial implementation evidence:** AndroidGuiAutomationContractTests genuine RED **0 passed / 4 failed** → GREEN **4 passed / 0 failed**; GuiTestProfileTests genuine RED **10 passed / 2 failed** → GREEN **12 passed / 0 failed**; Node evidence tests genuine RED **0 passed / 2 failed** → GREEN **3 passed / 0 failed**. The Node missing `test` import and Windows file-URL import issues were corrected before accepting RED. The first, pre-duplicate-correction bounded TEST_ONLY was **144 passed / 0 failed / 0 skipped** across AndroidGuiAutomationContractTests, GuiTestProfileTests, UiWorkflowContractTests, ReleaseNotesTests, AndroidPublishingScriptContractTests, and AndroidIconConfigurationTests; pure Node was **3 passed / 0 failed / 0 skipped**; pre/post `git diff --check` passed.
- **Historical duplicate-configuration correction evidence:** the XML-aware `AndroidGuiTestVariant_IsDeclaredExactlyOnce` regression produced genuine RED **4 passed / 1 failed** and GREEN **5 passed / 0 failed / 0 skipped**. The corrected bounded TEST_ONLY was **145 passed / 0 failed / 0 skipped**; pure Node remained **3 passed / 0 failed / 0 skipped**; pre/post `git diff --check` passed. Exactly one matching `KnownFirstAndroidGuiTest=true` / Debug / Android PropertyGroup remains.
- **Local PR-review correction evidence:** the initial five-finding correction IMPLEMENT recorded C# focused RED **5 passed / 2 failed** → GREEN **7 passed / 0 failed / 0 skipped**, and Node RED **3 passed / 2 failed** → GREEN **5 passed / 0 failed / 0 skipped**. Exact dependency-version characterization passed immediately and was not manufactured RED evidence. A subsequent local review found that an exact-port `Get-NetTCPConnection -LocalPort` query treated a normal free-port no-match as failure; the recovery focused RED was **6 passed / 1 failed / 0 skipped** and recovery GREEN was **7 passed / 0 failed / 0 skipped**. The launcher now treats a free port as an empty filtered listener set while genuine listener-query failures remain fail-closed.
- **Historical/superseded local correction validation:** the earlier bounded correction TEST_ONLY selected AndroidGuiAutomationContractTests, BuildIdentityServiceTests, GuiTestProfileTests, UiWorkflowContractTests, ReleaseNotesTests, AndroidPublishingScriptContractTests, and AndroidIconConfigurationTests: **159 passed / 0 failed / 0 skipped**. Pure Node evidence tests: **5 passed / 0 failed / 0 cancelled / 0 skipped / 0 todo**. Pre/post `git diff --check` passed, and the six implementation-file hashes were unchanged across testing.
- **Renewed-review two-MAJOR correction evidence:** the renewed static review found late SDK-style intermediate-path ordering and screenshot artifact/hash capture mismatch. Focused MAJOR-1 C# TDD recorded RED **6 passed / 2 failed** → GREEN **8 passed / 0 failed**; focused MAJOR-2 Node TDD recorded RED **5 passed / 2 failed** → GREEN **7 passed / 0 failed**. Final correction `4545fcc927ffdd787c548b447cbd21d087460e25` moves the GUI-test intermediate root to early app-project-specific `Directory.Build.props` evaluation and makes one native Release Notes capture supply both persisted PNG bytes and summary hashing.
