...
Automated GUI validation is [ROADMAP.md](ROADMAP.md) **priority 16** ("Automated GUI validation"), with merged P16-A foundation and merged P16-B S36 source mapping (P16-C Windows launcher integration pending). S36 remains fully manual in this matrix, and no matrix result may be inferred from source presence or contract tests.
...
| S14 | Settings | Open Settings in a disposable profile with no reset confirmation and, in Debug, diagnostic logging enabled. | Scroll through the full product order: UI language, appearance, new words per day, learning time zone, new learning day begins at, card directions, learning mode, Enhanced Term Recognition, online dictionary consent, portable data, DEBUG tools (Debug only), support, build identity, restore default settings, and reset. | Cards have consistent rhythm; the UI-language choices are exactly System, English, Deutsch, and Русский; System, Light, and Dark appearance choices are available; the learning-time-zone selector offers System plus 50 curated `(UTC±HH:mm) City` entries spanning inhabited UTC-11 through UTC+14 with clear vertical separation above the effective-timezone status line; the new-learning-day-begins-at control provides deterministic 24-hour hour (`00..23`) and minute (`00..59`) selectors with minute precision (no native AM/PM dependency); card directions and learning mode present defaults leftmost (Both directions; Automatic); restore default settings is visually neutral, positioned after Help & Support / build identity, and clearly separate from the destructive reset section; diagnostic actions alone are amber and labeled DEBUG. Help & Support exposes the release-note-history link and the functional Report a bug action. No Support KnownFirst control or "coming soon" placeholder panel is present in the rendered page or its accessibility tree. This row records their appearance in the actually rendered/installed build under test — that rendered confirmation has not yet been executed for any candidate, including the Settings GUI & Learning-Day Defaults — Slice 2A candidate on branch `feature/settings-gui-learning-day-defaults-v1`. | `kf-s14-settings-{viewport-id}-{platform}-{theme}-{ui-language}.png` | Yes |
...
| ID | State | Setup | Deterministic interaction | Expected application behavior | Screenshot requirement | Android physical-device requirement |
| --- | --- | --- | --- | --- | --- | --- |
| S36 | Release-note history — reopenable from Settings | Disposable profile on a build whose active version has an existing release-note entry. Launch once and dismiss the automatic What's New notice so its seen state is persisted. | Open Settings, locate the release-note-history action under Help & Support, and activate it. Return to Settings and activate it a second time. Then restart the application. | Help & Support exposes exactly one release-note-history action. Activating it navigates to the release-note page, whose header shows the localized release-note title rather than a "not found" title. The page lists Beta 13, Beta 12, Beta 11, and Beta 10 newest-first, each with its version heading and its own bullets. The second activation reopens the same complete history unchanged. After the restart the automatic What's New notice does **not** reappear, proving that opening the history neither reset nor re-triggered the one-time notice. Page header, entry spacing, and text remain usable and readable at every required viewport, with no horizontal overflow. No Support KnownFirst control or placeholder panel is present. | `kf-s36-release-note-history-{viewport-id}-{platform}-{theme}-{ui-language}.png` | Yes |

## Shell breakpoint-transition state (S37)

This state covers the compact-to-desktop transition while the navigation drawer is open. It requires a resizable application window or equivalent controlled viewport and has not been executed; source and contract-test coverage do not establish a rendered matrix pass.

| ID | State | Setup | Deterministic interaction | Expected application behavior | Screenshot requirement | Android physical-device requirement |
| --- | --- | --- | --- | --- | --- | --- |
| S37 | Navigation drawer — compact to desktop reconciliation | Open a long route in a disposable profile at a width below 800 CSS pixels, scroll the content away from its initial position, and open the navigation drawer. Confirm that the backdrop is present and background scrolling is locked. | Without closing the drawer, resize the viewport across the desktop boundary to at least 800 CSS pixels. Then scroll the route, invoke normal Back navigation once, and resize below 800 CSS pixels again. | Entering desktop mode closes the transient drawer state through the normal shell path: the desktop sidebar is persistent, the backdrop is gone, `<main>` is no longer inert, content scrolling works, and no stale drawer registration consumes Back navigation. Returning to compact width starts with the drawer closed and preserves normal content scrolling. No sidebar dimensions or desktop layout behavior change. | `kf-s37-shell-breakpoint-compact-open-{platform}-{theme}-{ui-language}.png` and `kf-s37-shell-breakpoint-desktop-reconciled-{platform}-{theme}-{ui-language}.png` | When a physical Android device supports a controlled resizable large-screen or multi-window transition; otherwise record breakpoint-not-applicable with device dimensions. |
