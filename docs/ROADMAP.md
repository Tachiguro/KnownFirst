...
| 15 | Portable merge integrity hardening | Current | Portable database export/import integrity is hardened in bounded packages before GUI automation begins. **KF-BACKUP-003 Package D, KF-BACKUP-004, KF-BACKUP-005A, and KF-BACKUP-005B are committed and merged** via PRs #76, #77, #79, and #81. 005B merge commit: `dc56e8412966ac32531c4b0358526582702d6d24`; `POST_MERGE_SYNC_ONLY` completed. Its final focused `TEST_ONLY` is **135 passed / 0 failed / 0 skipped** and final independent PR review was **0 BLOCKER / 0 MAJOR / 0 MINOR**; no GitHub CI evidence existed for the 005B head. Schema 10 and archive V2 remain unchanged. **KF-BACKUP-005C is the verified local candidate: its implementation, verification, review, and documentation work are complete locally except for the current correction/re-review; publication and merge lifecycle remain pending.** It still owns populated-target Active convergence and conflict safety. After its publication/merge lifecycle, the remaining Priority-15 residuals retain their listed ordering. All remaining portable-integrity residuals (`LegacyReviewSummaries` ordering, `Learning.Cards`/Sense `StableId` cross-installation ordering, and the legacy v1 planner's analogous synthesized label) remain open before GUI automation. Evidence for 005B is automated `TEST_ONLY` only — no `ValidateAll`, rendered GUI, platform build/runtime, Release build, device/emulator, APK/AAB, signing, publishing, or distribution evidence. |
...
14. **KF-BACKUP-005C:** Populated-target Active workflow convergence and conflict safety, including Active-vs-Active conflict handling. Implementation and bounded verification are complete on the local candidate branch; documentation review, commit, push, PR, manual merge, and post-merge synchronization remain pending. It is not merged `master` behavior. The existing Priority-15 residual ordering remains next: `LegacyReviewSummaries` canonical ordering, `Learning.Cards`/Sense `StableId` canonical ordering, then the legacy v1 planner-label residual.
