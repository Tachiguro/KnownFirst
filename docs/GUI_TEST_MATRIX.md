...
This is the repeatable manual visual and workflow matrix for human testers (and, in the future, deterministic automation — see "Automation status" below). It complements automated unit and contract tests. A passing build or contract test does not prove native rendering, safe-area behavior, focus behavior, or visual correctness.

This is a living document, not tied permanently to any single beta. States accumulate as features ship; a state is retired only when the feature it covers is removed from the product. The original state set (S01-S18) dates from Beta 7 hardening; Beta 10 states (S19-S29) cover portable data export/import and the What's New notice.
...
**No GUI automation currently exists for this matrix.** Every state below is executed manually today. [ROADMAP.md](ROADMAP.md) priority 4 ("Automated GUI validation") plans an Android-first deterministic automation effort (Appium/UiAutomator2) with a separate Windows feasibility spike; that work has not started. Do not report any row in this matrix as automated, and do not treat a passing automated unit/contract test as evidence for a row here.
...
## Beta 10 states (S19-S29): What's New and portable data export/import
...
| S27 | Portable Import — Populated-database refusal | On a disposable installation that already has at least one saved word or document, attempt Data Import with any valid archive. | Import is refused before any mutation, with a clear message that import requires an empty installation; existing data is unchanged. | `kf-s27-import-target-not-empty-{viewport-id}-{platform}-{theme}-{ui-language}.png` | Yes |
...
- Portable export/import states (S21-S29, scenario K) must run only against disposable profiles; never export from or import into a real user's installation as part of this matrix.
...
Do not mark a row visually passed unless its required visible state, overlap, spacing, scrolling, and focus behavior (or, for S19-S29, the described application behavior) were directly inspected.
