...
**Prioritization date:** 2026-08-23
...
The current product direction finishes real product functionality and testing: (1) Settings GUI & Learning-Day Defaults — Slice 2A is merged on `master` via PR #144 (Priority 19e below); Daily New-Word Limit & Learning-Day Infrastructure — Slice 1 is merged on `master` via PR #142 (Priority 19d below); German Enhanced Term Recognition Packages 1–5B are all merged on `master` (Priorities 17–19c below), (2) automated testing remains high priority, (3) user testing/feedback follows via pre-release Google Play Internal Testing distribution, for which Beta-13 Pre-AAB Release-Readiness has actively resumed under explicit user authorization to prepare a signed Google Play AAB, (4) Public-Release Readiness / Production promotion (Priority 20) remains **deferred** (privacy disclosures, website, Wikimedia attribution review, store materials) and resumes only when the user explicitly authorizes readiness for public store rollout.
...
3. **Beta-13 Pre-AAB Release-Readiness Gate (Active for Google Play Internal Testing):**
   - Source version `1.0.0-beta.13` (build 13) merged on `master` via PR #92; Database schema 12 active on `master` (PR #142).
   - Exact-master Pre-AAB Release-Readiness Gate ([docs/BUILD_AND_RELEASE.md](BUILD_AND_RELEASE.md) §7) is actively resuming on synchronized `master` to prepare a signed Google Play AAB for Internal Testing under explicit user authorization.
   - AAB creation is authorized once all mandatory Pre-AAB gate items pass; final AAB is not yet created; Google Play upload and Internal Testing distribution have not occurred; Public-Release Readiness / Production promotion remains separately deferred.
