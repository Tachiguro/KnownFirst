...
The duplicate-detection residual above is taken up by KF-BACKUP-004 in §22, which resolves the answer-variant half and records why `LearningSessionId` is deliberately kept out of merge identity. The mid-session policy is binding master behavior through 005B; the `LegacyReviewSummaries` correction is now binding master behavior through PR #85; the `Learning.Cards` ordering and legacy v1 planner-label residuals remain open.
