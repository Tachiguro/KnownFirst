...
KF-BACKUP-005B is current `master` behavior. It does not implement populated-target Active convergence; preview is `Blocked`, import is `Failed`, and both return `ActiveWorkflowUnsupported` without target mutation or executable merge/writer behavior. That boundary remains KF-BACKUP-005C.
