...
- Enforced authoritative post-onboarding online lookup consent governance and fail-closed privacy architecture: removed the former contextual consent disclosure from Prepare Words so Settings is the sole post-onboarding authority for granting or revoking consent; disabled the Automatic Online method card with explanatory copy and a Settings link when consent is OFF; provided a dedicated blocked-candidate state for existing Automatic Online sessions without batch mutation or data loss; and disabled lookup retry while consent is absent.
...
- Improved consistency and layout of the Words Review action bar: visually aligned the action bar with Words Preparation by grouping Known, Unknown, and Undo into a compact content-sized action group on the left, while positioning Discard import as a clearly separated destructive end action on the right; Undo is presented as a standard secondary button with clear disabled feedback; and localized Undo labels are shortened across English, German, and Russian while preserving destructive Discard import styling and confirmation.
...
- Fixed online lookup consent enforcement across lexical enrichment and preparation: added an authoritative transport authorization gate (`OnlineLookupAuthorizationGate` / `OnlineLookupAuthorizationHandler`) that fails fast on unauthorized outbound lexical HTTP requests, bound in-flight and background prefetch operations to authorization epochs with immediate cancellation on consent revocation, and ensured local SQLite lexical cache hits and existing prepared learning data remain usable without online consent.
- Aligned learning session next-due summary authority and due-time availability: corrected next-review timing so unadmitted new vocabulary and scheduled cards without active required assignments no longer produce misleading next-due times, and completed learning session summaries now react when a scheduled review becomes due by presenting an explicit primary Learn action without automatic session replacement (KF-LEARN-007).
