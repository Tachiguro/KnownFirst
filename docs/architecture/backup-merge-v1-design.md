...
The duplicate-detection residual above is taken up by KF-BACKUP-004 in §22, which resolves the answer-variant half and records why `LearningSessionId` is deliberately kept out of merge identity. The mid-session policy is binding master behavior through 005B; the `LegacyReviewSummaries` correction is now binding master behavior through PR #85. The `Learning.Cards` correction is the current unmerged feature-branch candidate described in §21.2; the legacy v1 planner-label residual remains open.
...
The remaining ordered open residuals after this correction are `Learning.Cards`/Sense `StableId` cross-installation ordering, then the legacy v1 planner analogous synthesized label.

### 21.2 Learning.Cards canonical ordering — current feature-branch candidate

**Lifecycle status:** implemented and bounded-validated on `fix/backup-v2-learning-cards-canonical-order-v1` at baseline `438d13a874247961e547d2deaa7da71f9f17cb33`, but not yet merged. This section records a current feature-branch candidate, not binding `master` behavior; its publication/review lifecycle must complete before the repository owner manually merges it. Earlier Package-D and KF-BACKUP-004 statements that this residual was open at those package boundaries remain historical facts.
...
**Candidate correction.** For cards whose Sense and vocabulary resolve, `BackupModelMapperV2` now orders semantic-first with existing identity policies: `FutureCardIdentity` (semantic Sense identity plus Direction), preferred-Meaning `ExactMeaningVariantIdentity`, and then every typed emitted Card-state distinction (`State`, UTC due/review/create/update ticks, interval, ease, review/lapse counts, nullable last-review/rating presence and value). Sense StableId is only late/final non-local material on this valid path. Preferred Meaning exact identity therefore participates before typed mutable state, and installation-random Sense GUID ordering is no longer the primary key.
...
**Evidence limitation:** this is bounded automated unit/integration/contract evidence only. It is not ALL_AUTOMATED, ValidateAll, GitHub CI, platform/runtime build, rendered-GUI/device, package, signing, publishing, or distribution evidence. It also does not establish universal whole-archive byte equality: unrelated installation-random StableIds elsewhere in an otherwise equivalent archive remain an invalid general byte-equality oracle. The next ordered Priority-15 residual is the legacy v1 planner analogous synthesized-label issue; GUI automation remains after Priority 15.
