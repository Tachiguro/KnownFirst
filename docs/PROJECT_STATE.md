...
**State source:** current `master` baseline — `a5a9e734af0db0639a38471433748e334ca34d65` (PR #92 merge commit). This is immutable historical merge evidence, not a permanent claim about the literal current `master` HEAD or a frozen future Pre-AAB candidate; the exact current `master` HEAD and PR state remain live GitHub/Git facts discovered dynamically per [docs/NEW_CHAT_BOOTSTRAP.md](NEW_CHAT_BOOTSTRAP.md).
**Current merged lifecycle:** PR #92 merged the `1.0.0-beta.13` / build `13` release candidate (final PR head `774b2245f64a986fe004f4ebd3288747642bdb0f`) into `master`, and its local `POST_MERGE_SYNC_ONLY` completed exactly once; it must not be repeated. PR #91 previously merged the P16-A Android GUI-automation source foundation and local `POST_MERGE_SYNC_ONLY` completed. PR #89's Occurrence action-key correction remains binding `master` behavior.
