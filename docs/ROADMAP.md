...
### Beta-13 release-candidate preparation (current release preparation)

A committed and pushed `1.0.0-beta.13` / build `13` release candidate is proposed by open, unmerged PR #92 from `release/1.0.0-beta.13-candidate-v1` (base `master` `810a7bcd0979c2f482135c07b56f834204b53118`); its initial candidate/PR-opening commit is `576865f9ed17f94c5f68aaa466f9bbf0244edb84`. The first remote review found lifecycle-documentation corrections only, which this DOCUMENT_ONLY package addresses; merge and all release-readiness gates remain pending. The candidate packages the user-visible capability merged to `master` since the confirmed `1.0.0-beta.12` distribution: populated-target portable import with read-only preview and merge/no-change outcomes, hardened backup/import merge safety (Priority 15), Schema-10 Active-learning-session portable preservation/resume (KF-BACKUP-005A/005B/005C), and the reopenable release-note history (Milestone 14B). Implementation evidence: focused genuine RED `7/36/0/43` → GREEN `43/0/0/43`; bounded affected/regression `TEST_ONLY` `138/0/0/138`; see [docs/releases/1.0.0-beta.13.md](releases/1.0.0-beta.13.md). This is **Internal Testing candidate preparation only** — it does not establish public Google Play promotion readiness, which remains governed by Priority 17 below.
