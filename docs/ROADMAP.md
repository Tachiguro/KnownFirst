...
The current product direction finishes real product functionality first: (1) Settings GUI & Learning-Day Defaults — Slice 2A is merged on `master` via PR #144 (Priority 19e below); Daily New-Word Limit & Learning-Day Infrastructure — Slice 1 is merged on `master` via PR #142 (Priority 19d below); German Enhanced Term Recognition Packages 1–5B are all merged on `master` (Priorities 17–19c below), (2) automated testing remains high priority, (3) user testing/feedback and UX cleanup follow once functionality is strong, (4) Public-Release Readiness / AAB creation (Priority 20) is **deferred** — not cancelled — and resumes only when the user explicitly authorizes/signals readiness to return to it.
