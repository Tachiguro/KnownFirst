...
- **Standing delegation covers routine lifecycle writes:** Under the user's standing orchestration delegation (defined in [docs/PROMPT_AND_TASK_ROUTING.md](docs/PROMPT_AND_TASK_ROUTING.md)), commit, push, and pull-request creation/update for an already established, bounded work package do not require a fresh per-operation user authorization message; ChatGPT may issue the next isolated `COMMIT_ONLY`, `PUSH_ONLY`, or `PR_ONLY` prompt after evaluating the prior phase's result.
