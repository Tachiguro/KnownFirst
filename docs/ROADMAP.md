...
| 16 | Automated GUI validation | Planned | Next planned milestone: Android-first deterministic GUI automation (Appium/UiAutomator2); Windows automation launcher integration. No scoped Priority-16 `PLAN_ONLY`, implementation branch, or implementation has started. |
...
**Portable merge integrity hardening (Priority 15) is complete and binding `master` behavior.** PR #89 merged the Occurrence action-key correction (implementation `edbb49a87ff3f37337c413111a60f6cfa6805b88`, feature head `d45a7e8fad533ddda5dda425356bf2095e8bafb6`, merge commit `49d25cb8d7d113d1f0b1826369d9105a37d9207b`) after focused TDD **RED 0/2/0 → GREEN 2/0/0**, bounded `TEST_ONLY` **257/0/0**, independent review **0 BLOCKER / 0 MAJOR / 0 MINOR / 0 NIT**, and `POST_MERGE_SYNC_ONLY`. The legacy LearningReview label is not a current production-writer defect on the V1-to-V2 route. Priority 16 is now next, remains Planned, and has not begun its own scoped package. See [CURRENT_WORK.md](CURRENT_WORK.md) for lifecycle state.
...
16. **Occurrence action-key correction:** merged and binding `master` behavior through PR #89 (implementation `edbb49a87ff3f37337c413111a60f6cfa6805b88`, feature head `d45a7e8fad533ddda5dda425356bf2095e8bafb6`, merge commit `49d25cb8d7d113d1f0b1826369d9105a37d9207b`; `POST_MERGE_SYNC_ONLY` complete). It uses the shared lookup-only `SourceMaterialArchiveId:Occurrence.Order` key rather than colliding `SentenceId:VocabularyId`; semantic occurrence identity and compatibility contracts are unchanged. Focused TDD **0/2/0 → 2/0/0**; bounded TEST_ONLY **257/0/0**; independent review **0 BLOCKER / 0 MAJOR / 0 MINOR / 0 NIT**. Priority 15 is Committed; Priority 16 remains Planned and not started.
