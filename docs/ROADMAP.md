...
The current product direction finishes real product functionality first: (1) German Enhanced Term Recognition — Packages 1–4 are merged on `master` via PR #134 (Priorities 17–19 below); Package 5A (derived-term post-review lifecycle integrity, Priority 19a below) is independently reviewed and approved (0 BLOCKER / 0 MAJOR / 0 MINOR) on branch `fix/german-derived-lifecycle-integrity-v1`, pending `COMMIT_ONLY`, a fresh exact-HEAD `FULL_VALIDATION` pass, and merge; further German work (Package 5A-2, Package 5B) remains planned, (2) automated testing remains high priority, (3) user testing/feedback and UX cleanup follow once functionality is strong, (4) Public-Release Readiness / AAB creation (Priority 20) is **deferred** behind that sequence — not cancelled — and resumes only when the user explicitly authorizes/signals readiness to return to it.
...
| 19a | German Enhanced Term Recognition — Package 5A (Derived-Term Post-Review Lifecycle Integrity) | Current | Retains a derived candidate's `ReviewCandidateEntity`/`DerivedTermEvidenceEntity` past review completion while the word stays Unknown, so Preparation can still recover real source-compound context; protects the retained evidence's Document/SentenceSpan from generic maintenance cleanup; cleans retained state up on MarkKnown/Exclude; filters the portable V2 export so a retained-evidence-owning candidate never leaks as a provenance-less item. No schema/archive-format change. Independently reviewed and approved (0 BLOCKER / 0 MAJOR / 0 MINOR) on branch `fix/german-derived-lifecycle-integrity-v1`; not yet committed, validated by `FULL_VALIDATION`, or merged. See [docs/CURRENT_WORK.md](CURRENT_WORK.md), [docs/PROJECT_STATE.md](PROJECT_STATE.md), [docs/DATABASE_CONTRACT.md](DATABASE_CONTRACT.md). |
| 19b | German Enhanced Term Recognition — Package 5A-2 (Portable/Cross-Installation Derived-Evidence Transport) | Planned | Portable/cross-installation transport of derived-term provenance/context where required — design must cover portable archive/restore and populated-target merge semantics without inventing unsafe identities. Package 5A intentionally does not serialize `DerivedTermEvidence`; cross-installation preservation remains unimplemented. Begins only after Package 5A merges. See [docs/BACKLOG.md](BACKLOG.md). |
| 19c | German Enhanced Term Recognition — Package 5B (Review Words Derivation-Source Indication) | Planned | Minimal Review Words UI/context indication so a user reviewing a derived component can understand it came from a source compound, without turning the product into a complex morphology inspector. Acceptance includes closing out any remaining end-to-end Known-suppression/Preparation/Learning regression coverage for German derived terms not already covered by Package 5A's review-added tests. Begins only after Package 5A merges. See [docs/BACKLOG.md](BACKLOG.md). |
