...
### G. ANDROID_GUI_AUTOMATION_FOUNDATION & S36 MAPPING (P16-A / P16-B)

- **Source-present harness:** `scripts/gui-tests/android/`; entry point `scripts/gui-tests/android/Invoke-AndroidGuiTest.ps1`; registered scenario `P16B-SettingsReleaseNotesHistory` (mapped to matrix state `S36`).
...
- **Current evidence boundary:**
  - *Validated:* Source/configuration contracts, profile-isolation unit behavior, runner/evidence pure logic, S36 scenario assertion structure (Beta 13/12/11/10 catalog ordering, localized header, second activation, `terminateApp`/`activateApp` restart, What's New non-reappearance, and obsolete support UI absence), and deterministic MSBuild property evaluation.
  - *Not Proven / Unexecuted:* The harness has **not** been executed on a live Android device or emulator; it does not prove Android platform compilation during testing, APK/AAB creation, package installation, Appium/UiAutomator2 runtime compatibility, Chromedriver/WebView runtime rendering, physical gesture navigation, matrix viewport coverage (D1-D3, T1, M1-M4), or matrix row S36 completion.
  - *Evidence Taxonomy:* `Source Mapping != Runtime Execution Pass != Full Matrix-Row Completion`. `matrixMapping: "S36"` establishes source-level scenario mapping only. `MANUAL_ANDROID_GUI` remains a distinct scope and automated source mapping does not replace manual matrix verification.
- **Historical P16-A implementation evidence:** AndroidGuiAutomationContractTests RED 0/4 → GREEN 4/0; GuiTestProfileTests RED 10/2 → GREEN 12/0; Node evidence tests RED 0/2 → GREEN 3/0; duplicate-configuration and listener corrections merged via PR #91.
- **P16-B implementation evidence:** Node RED 2/5 → Focused GREEN 7/0/0; AndroidGuiAutomationContractTests RED 7/1 → Focused GREEN 8/0/0; clean `git diff --check`.
