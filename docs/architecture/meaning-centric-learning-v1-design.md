...
**Status:** Binding architecture decision, **corrected twice** (two focused review passes), plus **Slice 0.1** (activation-sequencing review, §4.8, §6), itself corrected once more by a **final focused correction**: the entity model and migration contract below are unchanged, but the slice sequence is corrected so `DatabaseSchema.CurrentVersion` only becomes `8` after every write/read path that would otherwise violate the binding contract — and archive v2 — has already been adapted, merged, and (where it touches existing live entity classes or `InitializeAsync`) proven genuinely dormant against synthetic Schema-7/Schema-8 fixtures. See §4.8 for the precise dormancy definition, the pre-activation dual-schema rule, why archive v2 must precede activation, the binding `DormantFoundationThenActivation` strategy, and the rejected immediate-activation alternative. **Option B is decided, not provisional** — it supersedes the provisional recommendation in [structured-vocabulary-import-and-sense-learning.md](../plans/structured-vocabulary-import-and-sense-learning.md) §5 and the "approved meaning-centric model" recorded in [backup-merge-v1-design.md](backup-merge-v1-design.md) §16 (KF-BACKUP-002 Slice 3), which this document extends into a full entity model and migration contract. **No implementation exists beyond this architecture branch** (`feature/meaning-centric-architecture-v1`): no schema change, no new entity class, no migration code, no archive-format code, no service/UI change. This document is the contract later implementation slices must follow.
...
6a. **Create the corresponding `SenseAnswerVariantAssignmentEntity` rows — corrected: exactly one preferred assignment per existing card direction, chosen deterministically, never invented when no Meaning in the group has an answer for that direction.** For each `CardDirection` the word has an existing legacy `LearningCardEntity` row for (§4.2 step 8 below determines this per word): every `AnswerVariant` created in step 6 on the matching side (term-side/`SourceLanguage` for `MeaningToTerm`; explanation-side/`ExplanationLanguage` for `TermToMeaning`) receives an assignment `(SenseId, CardDirection, AnswerVariantId)` with `Requirement = AcceptedOnly` for every one of them ([Decision 12](#12-the-rule-that-providers-must-not-automatically-make-every-synonym-mandatory)) — never `Required`. Exactly one of these, per direction, is additionally `IsPreferred = true`, selected by the identical deterministic order as step 6: the representative Meaning's own contribution wins when it has one for that direction (the direct migrated equivalent of "this is the term/explanation you're expected to produce"); otherwise the contributing Meaning with the lowest legacy `Meaning.Id` wins (tie-broken by lowest normalized text under ordinal comparison, a case that cannot actually arise since `Meaning.Id` is already unique). If no Meaning in the group carries a candidate expression for a direction that does have an existing legacy card, no assignment — preferred or otherwise — is created for that direction; nothing is invented. No assignment of any kind is created for a direction that has no existing legacy card, since there is nothing yet to prefer or accept a display answer for.
...
| 4 | **Answer assignments, review-event fields, and progress replay, dual-schema compatible** | Slice 1 | `SenseAnswerVariantAssignmentEntity` read/write logic; `AnswerVariantProgressEntity` keyed by `(CardId, AnswerVariantId)`; `LearningSessionCardEntity`/`LearningReviewEntity` target/matched fields populated; the assignment-aware synonym-credit rule and the per-card reactivation rule implemented; `LearningService.PersistRating`/`RetireWordCards` branch on §4.8.2's schema-capability check: at Schema 7, still write `WordStatus.Learning`/`Mastered` and the automatic-progression columns exactly as today (unchanged regression behavior); at Schema 8 (fixture-only until Slice 6), stop writing them, use the Sense/`AnswerVariantProgressEntity` replacement instead, and retire only cards sharing `SenseId` (§4.8.3 bridge — this is the working replacement Slice 6 depends on to avoid regressing mastery/automatic-mode behavior). **No schema shape change beyond Slice 1, no archive change, no UI.** | A Schema-7 regression suite proves `PersistRating`/`RetireWordCards` are unchanged for a `user_version = 7` database; a Schema-8 fixture suite proves the same variant can be `Required` for one direction and `AcceptedOnly`/absent for the other on the same Sense with independently tracked progress, a matched-but-non-targeted synonym credits only a variant that is itself a valid `Required` assignment for that same card, a card reaches `Retired` only once every `Required` assignment for its own direction is mastered, no Schema-8 code path writes `WordStatus.Learning`/`Mastered` or the frozen automatic-progression columns, and only same-`SenseId` cards are retired together |
