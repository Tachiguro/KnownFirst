...
| 19c | German Enhanced Term Recognition — Package 5B (Review Words Derivation-Source Indication) | Planned — next planned German package | Minimal Review Words UI/context indication so a user reviewing a derived component can understand it came from a source compound, without turning the product into a complex morphology inspector. Acceptance includes closing out any remaining end-to-end Known-suppression/Preparation/Learning regression coverage for German derived terms not already covered by Package 5A's review-added tests. Its documented prerequisite — the Package 5A-2 merge — is now satisfied; implementation has not started. See [docs/BACKLOG.md](BACKLOG.md). |
