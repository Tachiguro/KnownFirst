...
- **Standing delegation covers routine lifecycle writes:** Under the user's standing orchestration delegation (defined in [docs/PROMPT_AND_TASK_ROUTING.md](docs/PROMPT_AND_TASK_ROUTING.md)), commit, mandatory candidate-HEAD FULL_VALIDATION, push, and pull-request creation/update for an already established, bounded work package do not require a fresh per-operation user authorization message; ChatGPT may issue the next isolated prompt (`COMMIT_ONLY`, `TEST_ONLY`, `PUSH_ONLY`, or `PR_ONLY`) after evaluating the prior phase's result.
