...
- **Standing delegation covers routine lifecycle writes:** Under the user's standing orchestration delegation (defined in [docs/PROMPT_AND_TASK_ROUTING.md](docs/PROMPT_AND_TASK_ROUTING.md)), the mandatory `DOCUMENT_ONLY` reconciliation, `COMMIT_ONLY`, mandatory candidate-HEAD `FULL_VALIDATION` (`TEST_ONLY`), `PUSH_ONLY`, and `PR_ONLY` for an already established, bounded work package do not require a fresh per-operation user authorization message; ChatGPT may issue the next isolated prompt after evaluating the prior phase's result. The mandatory `DOCUMENT_ONLY` phase is never optional or skippable for normal code/behavior packages: `COMMIT_ONLY` is blocked until `DOCUMENT_ONLY` has completed (or has explicitly reported `DOCUMENTATION_CURRENT_NO_CHANGES`). The normal code/behavior package lifecycle is: `PLAN_ONLY` → `IMPLEMENT` → focused verification → `REVIEW_ONLY` → mandatory `DOCUMENT_ONLY` → `COMMIT_ONLY` → exact-candidate-HEAD `FULL_VALIDATION` → `PUSH_ONLY` → `PR_ONLY` → manual user merge → `POST_MERGE_SYNC_ONLY`. Documentation-only packages omit `IMPLEMENT` and use: `PLAN_ONLY` → `DOCUMENT_ONLY` → `REVIEW_ONLY` → `COMMIT_ONLY` → `FULL_VALIDATION` → `PUSH_ONLY` → `PR_ONLY` → manual user merge → `POST_MERGE_SYNC_ONLY`.
