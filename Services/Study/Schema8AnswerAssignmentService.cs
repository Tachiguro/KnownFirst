...
public sealed class Schema8AnswerAssignmentService
{
        ...
        bool isPreferred)
    {
        ...
        if (LearningSchemaCapability.Resolve(connection) is not (LearningSchema8CapabilityResult or LearningSchema9CapabilityResult or LearningSchema10CapabilityResult or LearningSchema11CapabilityResult or LearningSchema12CapabilityResult))
        {
            throw Schema8LearningDataException.Create(
                Schema8LearningDataErrorCode.InvalidCardGraph,
                "Answer assignments can only be mutated against a validated Schema-8, Schema-9, Schema-10, Schema-11, or Schema-12 database.");
        }
