...
- **Compiler warning strictness & packaging parity:** The canonical Google Play packaging publisher compiles under `-warnaserror` with `-p:ILLinkTreatWarningsAsErrors=true`. To prevent false-positive validation passes where source compiler warnings pass validation but fail packaging, the mandatory `FULL_VALIDATION` / `ValidateAll` Android Release build step must enforce strict general compiler warnings as errors (`-warnaserror`), trimming warnings as errors (`-p:ILLinkTreatWarningsAsErrors=true`), and non-incremental fresh compilation (`--no-incremental`) while retaining `dotnet build`, `Release`, `net10.0-android`, `-m:1`, and `--no-restore` without creating package outputs. A successful Pre-AAB technical gate must not permit source/compiler warnings that the canonical AAB publisher will later reject.
...
- **Unpackaged runtime publisher:** `Platforms\Windows\Package.appxmanifest` sets `PublisherDisplayName` to the stable value `Tachiguro`. MAUI 10.0.20 derives unpackaged AppInfo publisher metadata from this element in the generated manifest; with ordinary application ID `com.tachiguro.knownfirst`, the expected app-data path is `%LOCALAPPDATA%\Tachiguro\com.tachiguro.knownfirst\Data`. This is source/toolchain-derived expectation, not observed runtime filesystem evidence; see [PROJECT_STATE.md](PROJECT_STATE.md#unpackaged-windows-app-data-identity-kf-windows-001) for the persistence baseline and unsupported historical-path boundary.
- **Development MSIX and future Store identity:** The source manifest retains `Identity/@Publisher="CN=User Name"` for the development MSIX identity, `Identity/@Name="maui-package-name-placeholder"`, and `Identity/@Version="0.0.0.0"` for MAUI substitution. The stable runtime publisher does not establish a Partner Center identity or signing evidence. Both the launcher and publishing script inspect the manifest via `scripts/packaging/windows-distribution-common.ps1` and classify it as `devidentity`. Current development MSIX output is **NOT** a Microsoft Store submission candidate until authoritative Partner Center values are supplied and separately validated under `KF-RELEASE-002` in [BACKLOG.md](BACKLOG.md).
...
- **Single source of truth:** `scripts/packaging/windows-distribution-common.ps1` authoritatively derives short commit prefixes, artifact filenames, signing markers (`unsigned`/`signed`), Store identity markers (`devidentity`/`storeidentity`), MSIX candidate selection, and ZIP file-entry counting. Both packaging scripts and the launcher dot-source this helper.
- **Runtime evidence boundary:** Automated source-contract tests and deterministic MSBuild evaluation verify packaging arguments, path isolation, version mapping, and safety guards. On 2026-08-16, the canonical `WindowsPortablePackage` action was executed from synchronized `master` commit `9e455d0e03494cac8e713cd4d16c66946124f852`, producing verified self-contained archive `artifacts\windows-portable\KnownFirst-1.0.0-beta.13-build13-win-x64-9e455d0.zip` (99,323,891 bytes, SHA-256 `cebd84824aa4e7909edb3a6e83c467573c3e245535ae585b6be832934a45a81e`, 647 archive entries) with confirmed presence of runtime payload markers (`KnownFirst.exe`, `KnownFirst.dll`, `hostfxr.dll`, `Microsoft.WindowsAppRuntime.Bootstrap.dll`) and 0 `BLAZOR102` or HEAD-resolution errors. Clean-PC installation/execution, MSIX package creation, certificate signing, and Store ingestion/update behavior remain unproven until separately authorized operations.

### Legacy Direct-Install Helper Limitation
- `scripts/packaging/publish-android-test-packages.ps1` publishes Release, BetaDiagnostic, and Debug APKs, but its artifact names contain hard-coded legacy Beta 6 labels and installation metadata.
