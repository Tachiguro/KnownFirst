...
| 15 | Portable merge integrity hardening | Current | Portable database export/import integrity is hardened in bounded packages before GUI automation begins. **KF-BACKUP-003 Package D, KF-BACKUP-004, KF-BACKUP-005A, KF-BACKUP-005B, and KF-BACKUP-005C are committed and merged** via PRs #76, #77, #79, #81, and #83. 005C merge commit: `bed54d01624e80ca6dd5adf8af097e64fe33e588`; `POST_MERGE_SYNC_ONLY` completed. Schema 10 and archive V2 remain unchanged. The current `LegacyReviewSummaries` correction is an implemented candidate on `fix/legacy-review-summaries-canonical-ordering-v1`, based on `9cc6bbe08201107068f69ad1dd397f188499e69d`, and is not yet committed, pushed, PR-reviewed on GitHub, or merged. Its evidence is focused RED **1 failed / 0 passed** → GREEN **1 passed / 0 failed / 0 skipped**, independent review **0 BLOCKER / 0 MAJOR / 0 MINOR / 0 NIT**, and bounded affected/regression `TEST_ONLY` **110 passed / 0 failed / 0 skipped**. The remaining open sequence is (1) `Learning.Cards`/Sense `StableId` cross-installation ordering, (2) the legacy v1 planner's analogous synthesized label; GUI automation remains after Priority 15. Candidate evidence is bounded automated unit/integration/contract evidence only; it is not ALL_AUTOMATED, ValidateAll, GitHub CI, platform/runtime, Release-build, rendered-GUI, device/emulator, APK/AAB, signing, publishing, or distribution evidence. |
...
**Portable merge integrity hardening (priority 15) is the current milestone, and precedes automated GUI validation.** KF-BACKUP-003 Package D is merged via PR #76, KF-BACKUP-004 via PR #77, KF-BACKUP-005A via PR #79, KF-BACKUP-005B via PR #81, and **KF-BACKUP-005C (populated-target Active workflow convergence and conflict safety) via PR #83** (merge commit `bed54d01624e80ca6dd5adf8af097e64fe33e588`); `POST_MERGE_SYNC_ONLY` completed successfully for all. The current `LegacyReviewSummaries` correction is an uncommitted, feature-branch candidate on `fix/legacy-review-summaries-canonical-ordering-v1`, independently reviewed and bounded-`TEST_ONLY`-validated (**110/0/0**), not a merged master fact. The remaining residual order is `Learning.Cards`/Sense `StableId` ordering, then the legacy v1 planner analogous synthesized label. See [CURRENT_WORK.md](CURRENT_WORK.md) for lifecycle state.
...
14. **KF-BACKUP-005C:** Populated-target Active workflow convergence and conflict safety, including Active-vs-Active conflict handling, is merged and binding `master` behavior via PR #83 (merge commit `bed54d01624e80ca6dd5adf8af097e64fe33e588); `POST_MERGE_SYNC_ONLY` completed successfully. At the historical KF-BACKUP-005C boundary, the residual sequence began with `LegacyReviewSummaries` canonical ordering, followed by `Learning.Cards`/Sense `StableId` canonical ordering and the legacy v1 planner-label residual. `LegacyReviewSummaries` is now an implemented, independently reviewed, bounded-test-validated feature-branch candidate and is not yet merged; Priority 15 remains Current. The current remaining residual order after this candidate is (1) `Learning.Cards`/Sense `StableId` cross-installation ordering, then (2) the legacy v1 planner analogous synthesized-label residual.
