...
**State source:** current `master` baseline — `810a7bcd0979c2f482135c07b56f834204b53118` (PR #91 merge commit). The exact current `master` HEAD and PR state remain live GitHub/Git facts discovered dynamically per [docs/NEW_CHAT_BOOTSTRAP.md](NEW_CHAT_BOOTSTRAP.md).
**Current merged lifecycle:** PR #91 merged the P16-A Android GUI-automation source foundation and local `POST_MERGE_SYNC_ONLY` completed. PR #89's Occurrence action-key correction remains binding `master` behavior.
...
**Active PR candidate — Beta 13:** the `1.0.0-beta.13` / build `13` release candidate is committed and pushed on `release/1.0.0-beta.13-candidate-v1` from `master` `810a7bcd0979c2f482135c07b56f834204b53118`. Its original reviewed candidate and PR-opening commit is `576865f9ed17f94c5f68aaa466f9bbf0244edb84`; PR #92 is open and unmerged, so Beta 13 is not yet binding `master` identity. Local independent pre-commit review passed **0 BLOCKER / 0 MAJOR / 0 MINOR / 1 NIT**. The first independent remote review of PR #92 found **0 BLOCKER / 1 MAJOR / 1 MINOR / 0 NIT**, limited to stale lifecycle documentation and no product-code defect; this DOCUMENT_ONLY package corrects those findings. Focused genuine RED **7 failed / 36 passed / 0 skipped / 43 total** → identical GREEN **43 passed / 0 failed / 0 skipped / 43 total**; bounded affected/regression `TEST_ONLY` **138 passed / 0 failed / 0 skipped / 138 total** with unchanged pre/post implementation-file SHA-256 hashes and passing `git diff --check`. Candidate evidence record: [docs/releases/1.0.0-beta.13.md](releases/1.0.0-beta.13.md). The last confirmed external distribution remains `1.0.0-beta.12` / build 12 (2026-07-30); that evidence predates and does not validate current Schema-10 populated-target merge or Active-workflow portability.
...
| --- | --- |
| Project | KnownFirst |
| Source Version (`master`) | `1.0.0-beta.12` (build 12) |
| Open PR release-candidate version (`release/1.0.0-beta.13-candidate-v1`, PR #92) | `1.0.0-beta.13` (build 13) — committed and pushed, but not yet binding `master` identity; see "Active PR candidate — Beta 13" above and [docs/releases/1.0.0-beta.13.md](releases/1.0.0-beta.13.md) |
