...
**State source:** current `master` baseline — `8eeaea58d87f9cfeb28cc4fc2520e5b277bb2526` (PR #85 merge commit). The exact current `master` HEAD and PR state remain live GitHub/Git facts discovered dynamically per [docs/NEW_CHAT_BOOTSTRAP.md](NEW_CHAT_BOOTSTRAP.md).
**Current merged package:** `LegacyReviewSummaries` canonical ordering correction is binding `master` behavior via PR #85 (feature head `baf5fcda0a017c1492a08dac730d683c1554784d`, merge commit `8eeaea58d87f9cfeb28cc4fc2520e5b277bb2526`); focused TDD RED **1 failed / 0 passed** → GREEN **1 passed / 0 failed / 0 skipped**; bounded affected/regression TEST_ONLY **110 passed / 0 failed / 0 skipped**; implementation review **0 BLOCKER / 0 MAJOR / 0 MINOR / 0 NIT**; `POST_MERGE_SYNC_ONLY` completed successfully.
**Separate active-branch current work:** the Priority-15 V2 `Learning.Cards` canonical-ordering correction is implemented and bounded-validated on `fix/backup-v2-learning-cards-canonical-order-v1` at baseline `438d13a874247961e547d2deaa7da71f9f17cb33`, but is not yet merged. It is not a production capability or binding `master` behavior; the feature-branch publication/review lifecycle must complete before the repository owner manually merges it.
...
- KF-BACKUP-003 Package D is merged via PR #76, KF-BACKUP-004 via PR #77, KF-BACKUP-005A via PR #79, KF-BACKUP-005B via PR #81 (merge commit `dc56e8412966ac32531c4b0358526582702d6d24`), KF-BACKUP-005C via PR #83 (merge commit `bed54d01624e80ca6dd5adf8af097e64fe33e588`), and `LegacyReviewSummaries` canonical ordering via PR #85 (merge commit `8eeaea58d87f9cfeb28cc4fc2520e5b277bb2526`); `POST_MERGE_SYNC_ONLY` completed successfully for all. The current master baseline is `8eeaea58d87f9cfeb28cc4fc2520e5b277bb2526`.
- Historical KF-BACKUP-005B package baseline was `dc56e8412966ac32531c4b0358526582702d6d24` (PR #81 merge commit).

**Active feature-branch candidate (not master):**

- The V2 `Learning.Cards` candidate replaces installation-random Sense-`StableId`-first ordering with semantic-first ordering for valid/resolved cards, including preferred-Meaning exact identity and typed emitted Card state; Sense StableId remains only late/final non-local material. It preserves mapper non-validation through a deterministic content-based fallback for malformed/unresolved-Sense snapshots, preserves multiplicity, and introduces no grouping/deduplication.
...
- Schema 10, outer archive V2, DTOs, migrations/validators, V1 compatibility, merge and scheduler semantics, UI, transport, synchronization, public status/error codes, and persistence contracts remain unchanged. The next open residual is the legacy v1 planner analogous synthesized-label issue.
