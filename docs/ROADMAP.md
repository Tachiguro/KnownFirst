...
12. **Schema-10 Stable Learning-Workflow Identity (KF-BACKUP-005A):** Implemented and validated on feature branch `feature/schema10-stable-learning-workflow-identity-v1` (`ALL_AUTOMATED` 1812/0/0; Windows Debug build green); not yet merged to `master`. Introduces immutable `StableId` columns for `LearningSessions`/`LearningSessionCards`, deterministic Completed bootstrap (SHA-256, 64 chars), one-time Active GUID bootstrap (32 chars), archive/source ≤9 Completed compatibility, source ≥10 StableId validation. Active portable-workflow continuation is deferred to KF-BACKUP-005B.
