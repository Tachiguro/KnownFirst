...
**Status:** Historical Schema-8 architecture decision and implementation record. At the recorded checkpoint, Slices 1–8 were merged to `master` (PR #44); Slice 9 (import preview UI, localized handling, corrected LearningSession identity) was implemented and validated on `feature/meaning-slice9-import-preview-ui` awaiting manual merge. The entity model and migration contract below describe that Schema-8 state. See §4.8 for the activation-sequencing rules (`DormantFoundationThenActivation` strategy) applied during Slices 1–6 merge sequence: every write/read path was adapted, merged, and proven dormant against synthetic Schema-7/Schema-8 fixtures before `DatabaseSchema.CurrentVersion` became `8` (Slice 6). Archive v2 (Slice 2) preceded activation (Slice 6). **Option B was decided and implemented** — see [backup-merge-v1-design.md](backup-merge-v1-design.md) §16 (KF-BACKUP-002 Slice 3) for the product model this document extends into the full entity model and migration contract.

**Current authority and historical scope:** The semantic Word/Sense/Meaning/AnswerVariant separation remains relevant. This document's legacy scheduling, mastery/retirement, queue pruning, Sense mastery rollup, reactivation rules, and activation chronology are historical Schema-8 contracts, superseded for current production by the [Schema-13 production contract](../DATABASE_CONTRACT.md#schema-13-production-contract-kf-fsrs-003). Current scheduling is FSRS-based; interaction progress and explicit clean controls are separate, and Schema-13 review replay does not synthesize `Mastered`, `Retired`, or mastery extensions. Historical wording such as “binding,” “current,” or “future” below applies to that recorded design stage, not to the current runtime. This does not resolve `KF-LEARN-004`, `KF-LEARN-005`, `KF-METRIC-001`, or the separate Definition/Translation learning-target decision `KF-LEARN-010`.
...
**Historical Schema-8 mastery/retirement (superseded for Schema 13):** progress was per `(CardId, AnswerVariantId)`. A card was eligible to reach `Retired` only once **every** `SenseAnswerVariantAssignmentEntity` row with `Requirement = Required` for that card's own `(SenseId, Direction)` had a corresponding `AnswerVariantProgressEntity` row (keyed by that same `CardId`) with `IsMastered = true`. A card with intentionally zero `Required` assignments was not automatically mastered and was not eligible to newly retire.

**Historical Sense rollup (superseded for Schema 13):** `SenseEntity.Status = Mastered` (§2.2) required **every existing `LearningCardEntity` for that Sense (across whichever directions had been created) to be `Retired`**. One direction could retire independently from its sibling; another Sense of the same Word was unaffected. `WordEntity.Status` was not the persisted Sense mastery rollup. Current Schema-13 replay does not produce this rollup.

**Historical reactivation rule (binding only for the Schema-8 design; superseded for Schema 13):** adding a new `Required` assignment to a retired card's direction, or promoting an existing `AcceptedOnly` assignment to `Required`, reactivated **that card** and recomputed the affected Sense's legacy status. The design left re-entry scheduling mechanics to §6 slice 5 and prohibited reactivating a legacy card merely because migration introduced the assignment representation. This history does not define current FSRS re-entry, clean-control reversal, or a new mastery concept.
...
1. **Historical reactivation scheduling mechanics** (§2.9): the Schema-8 design bound newly-`Required` assignments to legacy card reactivation and left precise mechanics to §6 slice 5. This is superseded for Schema 13, not an open current-production reactivation requirement.
