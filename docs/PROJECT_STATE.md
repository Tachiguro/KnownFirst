...
**State source:** current `master` baseline — `133d34366204979d2905c665370531547a7a0b98` (PR #88 merge commit). The exact current `master` HEAD and PR state remain live GitHub/Git facts discovered dynamically per [docs/NEW_CHAT_BOOTSTRAP.md](NEW_CHAT_BOOTSTRAP.md).
**Current merged lifecycle:** PR #87's `Learning.Cards` correction remains binding master behavior; PR #88 merged feature head `4fc8b15f0861ab264ef7518ef66e810b0cf3c15c` and completed its documentation closure, followed by `POST_MERGE_SYNC_ONLY`.
**Current Priority-15 candidate:** the Occurrence action-key correction is implemented and bounded-validated on `fix/backup-v1-planner-action-key-v1`, but is uncommitted, not independently reviewed, not merged, and not binding master behavior. It corrects the production-representable collision of the former `SentenceId:VocabularyId` Occurrence action key with the shared lookup-only key `SourceMaterialArchiveId:Occurrence.Order`. Focused TDD: **0 passed / 2 failed / 0 skipped** → **2 passed / 0 failed / 0 skipped**; bounded TEST_ONLY: **257 passed / 0 failed / 0 skipped**. Priority 15 remains Current; Priority 16 remains Planned.
...
- Independent review found **0 BLOCKER / 0 MAJOR / 0 MINOR / 0 NIT**. Schema 10, outer archive V2, DTOs, migrations/validators, V1 compatibility, merge and scheduler semantics, UI, transport, synchronization, public status/error codes, and persistence contracts remain unchanged. PR #88 subsequently completed the PR #87 documentation closure; the current candidate is the Occurrence action-key correction, while Priority 16 remains Planned.
