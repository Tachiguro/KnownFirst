...
1. Functional (or explicitly removed) Support KnownFirst and Report a bug controls. **Report a bug is implemented** on `feature/report-bug-email-v1` as a functional email action under Settings → Help & Support with copy-address fallback, while Support KnownFirst remains absent without placeholders. Confirming behavior in an actual Release build and AAB belongs to the mandatory pre-AAB validation gate in [BUILD_AND_RELEASE.md](BUILD_AND_RELEASE.md).
