...
public static class Schema8BackupSnapshotRepository
{
    ...
    /// Recognition Package 5A), and — Package 5A-2 — with the actual owned <c>DerivedTermEvidenceEntries</c>
    /// rows themselves. Issued as its own statement, exactly like <see cref="WithSchema10LearningIdentities"/>,
    /// so the Schema-8/9/10 capture queries above stay byte-for-byte valid against a database that has no
    /// <c>DerivedTermEvidenceEntries</c> table at all. Callers must have resolved a
    /// <see cref="Services.DataSafety.ValidatedSchema11Capability"/> first. Both results are restricted to
    /// owning ids already present in <paramref name="snapshot"/>'s captured <c>ReviewCandidates</c> — a row
    /// already excluded by a portable-export filter (e.g. an Active-session candidate under
    /// <see cref="CapturePortableSnapshot"/>) is never spuriously reintroduced here.
    /// </summary>
        ...
        SQLiteConnection connection, Schema8BackupSnapshot snapshot)
    {
        ...
        var allEvidence = connection.Table<DerivedTermEvidenceEntity>().ToList();
        var ownedEvidence = allEvidence.Where(entry => candidateIds.Contains(entry.ReviewCandidateId)).ToList();
        var owningIds = ownedEvidence.Select(entry => entry.ReviewCandidateId).ToHashSet();

        return snapshot with
        {
            DerivedTermEvidenceOwningReviewCandidateIds = owningIds,
            DerivedTermEvidence = ownedEvidence
        };
    }
