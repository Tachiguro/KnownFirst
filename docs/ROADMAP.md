...
| 15 | Portable merge integrity hardening | Current | Portable database export/import integrity is hardened in bounded packages before GUI automation begins. **KF-BACKUP-003 Package D, KF-BACKUP-004, KF-BACKUP-005A, KF-BACKUP-005B, KF-BACKUP-005C, and the `LegacyReviewSummaries` canonical ordering correction are committed and merged** via PRs #76, #77, #79, #81, #83, and #85. `LegacyReviewSummaries` merge commit: `8eeaea58d87f9cfeb28cc4fc2520e5b277bb2526` (feature head `baf5fcda0a017c1492a08dac730d683c1554784d`); `POST_MERGE_SYNC_ONLY` completed. Schema 10 and archive V2 remain unchanged. Evidence for the `LegacyReviewSummaries` correction: focused RED **1 failed / 0 passed** → GREEN **1 passed / 0 failed / 0 skipped**, independent review **0 BLOCKER / 0 MAJOR / 0 MINOR / 0 NIT**, and bounded affected/regression `TEST_ONLY` **110 passed / 0 failed / 0 skipped**. The remaining open sequence is (1) `Learning.Cards`/Sense `StableId` cross-installation ordering, (2) the legacy v1 planner's analogous synthesized label; GUI automation remains after Priority 15. Evidence is bounded automated unit/integration/contract evidence only; it is not ALL_AUTOMATED, ValidateAll, GitHub CI, platform/runtime, Release-build, rendered-GUI, device/emulator, APK/AAB, signing, publishing, or distribution evidence. |
...
**Portable merge integrity hardening (priority 15) is the current milestone, and precedes automated GUI validation.** KF-BACKUP-003 Package D is merged via PR #76, KF-BACKUP-004 via PR #77, KF-BACKUP-005A via PR #79, KF-BACKUP-005B via PR #81, KF-BACKUP-005C via PR #83, and **the `LegacyReviewSummaries` canonical ordering correction via PR #85** (feature head `baf5fcda0a017c1492a08dac730d683c1554784d`, merge commit `8eeaea58d87f9cfeb28cc4fc2520e5b277bb2526`); `POST_MERGE_SYNC_ONLY` completed successfully for all. Bounded affected/regression TEST_ONLY for the `LegacyReviewSummaries` correction: **110/0/0**. The remaining residual order is (1) `Learning.Cards`/Sense `StableId` ordering, (2) the legacy v1 planner analogous synthesized label. Neither residual has started. See [CURRENT_WORK.md](CURRENT_WORK.md) for lifecycle state.
