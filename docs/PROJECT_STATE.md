...
KF-BACKUP-005B is current `master` behavior. Its historical populated-target Active boundary remains true on `master`. A verified KF-BACKUP-005C local candidate now implements the bounded populated-target convergence path but is uncommitted, unpushed, without a PR, and not binding master behavior.
