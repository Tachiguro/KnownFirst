...
| 15 | Portable merge integrity hardening | Current | Portable database export/import integrity is hardened in bounded packages before GUI automation begins. **KF-BACKUP-003 Package D, KF-BACKUP-004, KF-BACKUP-005A, and KF-BACKUP-005B are committed and merged** via PRs #76, #77, #79, and #81. 005B merge commit: `dc56e8412966ac32531c4b0358526582702d6d24`; `POST_MERGE_SYNC_ONLY` completed. Its final focused `TEST_ONLY` is **135 passed / 0 failed / 0 skipped** and final independent PR review was **0 BLOCKER / 0 MAJOR / 0 MINOR**; no GitHub CI evidence existed for the 005B head. Schema 10 and archive V2 remain unchanged. **KF-BACKUP-005C is a published candidate in open PR #83: implementation, verification, review, and documentation work are complete except for the current correction/re-review; final approval and merge lifecycle remain pending.** It still owns populated-target Active convergence and conflict safety. After its merge lifecycle, the remaining Priority-15 residuals retain their listed ordering. All remaining portable-integrity residuals (`LegacyReviewSummaries` ordering, `Learning.Cards`/Sense `StableId` cross-installation ordering, and the legacy v1 planner's analogous synthesized label) remain open before GUI automation. Evidence for 005B is automated `TEST_ONLY` only — no `ValidateAll`, rendered GUI, platform build/runtime, Release build, device/emulator, APK/AAB, signing, publishing, or distribution evidence. |
...
**Portable merge integrity hardening (priority 15) is the current milestone, and precedes automated GUI validation.** KF-BACKUP-003 Package D is merged via PR #76, KF-BACKUP-004 via PR #77, KF-BACKUP-005A via PR #79, and **KF-BACKUP-005B (portable Active workflow export and restore into an empty target) via PR #81** (feature commit `e8236bba3d23e942014e6979b661e0c77a2a3bdd`, merge commit `dc56e8412966ac32531c4b0358526582702d6d24`); `POST_MERGE_SYNC_ONLY` completed successfully for all. 005B focused final `TEST_ONLY` is **135 passed / 0 failed / 0 skipped**; no GitHub CI evidence existed for its head. **KF-BACKUP-005C is implemented and verified as a published candidate in open PR #83; final correction/review and repository-owner manual-merge lifecycle remain, so it is not binding `master` behavior.** Remaining portable-integrity residuals stay open before GUI automation begins. See [CURRENT_WORK.md](CURRENT_WORK.md) for the active package state.
...
13. **KF-BACKUP-005B:** Completed and merged via PR #81 (merge commit `dc56e8412966ac32531c4b0358526582702d6d24`); `POST_MERGE_SYNC_ONLY` completed successfully. Ordinary Schema-10 export carries the Active workflow's durable queue and committed review history, and empty-target restore resumes it from the last committed application/database state. Final focused `TEST_ONLY`: **135 passed / 0 failed / 0 skipped**. Completed workflow regression coverage remains green. Schema-8/9 stay Completed-only. Populated-target Active archives remain fail-closed.
14. **KF-BACKUP-005C:** Populated-target Active workflow convergence and conflict safety, including Active-vs-Active conflict handling. Implementation, bounded verification, and documentation are complete as a published candidate in open PR #83; final correction/review, repository-owner manual merge after approval, and post-merge synchronization remain. It is not merged `master` behavior. The existing Priority-15 residual ordering remains next: `LegacyReviewSummaries` canonical ordering, `Learning.Cards`/Sense `StableId` canonical ordering, then the legacy v1 planner-label residual.
