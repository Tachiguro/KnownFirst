...
| 15 | Portable merge integrity hardening | Current | Portable database export/import integrity is hardened in bounded packages before GUI automation begins. **KF-BACKUP-003 Package D, KF-BACKUP-004, KF-BACKUP-005A, KF-BACKUP-005B, and KF-BACKUP-005C are committed and merged** via PRs #76, #77, #79, #81, and #83. 005C merge commit: `bed54d01624e80ca6dd5adf8af097e64fe33e588`; `POST_MERGE_SYNC_ONLY` completed. Its focused `Schema10ActiveArchive` evidence is **8 passed / 0 failed / 0 skipped**; its controlled affected/regression scope is **254 passed / 0 failed / 0 skipped** with Workers=1 and **254 passed / 0 failed / 0 skipped** with Workers=8; final relevant reviews: **0 BLOCKER / 0 MAJOR / 0 MINOR**. Schema 10 and archive V2 remain unchanged. 005C owns binding populated-target Active convergence and conflict safety. Priority 15 remains active for the documented residual sequence: (1) `LegacyReviewSummaries` ordering, (2) `Learning.Cards`/Sense `StableId` cross-installation ordering, (3) the legacy v1 planner's analogous synthesized label. These remain open before GUI automation. 005C evidence is bounded automated unit/integration/persistence/contract evidence only; it is not ALL_AUTOMATED, ValidateAll, GitHub CI, platform/runtime, Release-build, rendered-GUI, device/emulator, APK/AAB, signing, publishing, or distribution evidence. |
...
**Portable merge integrity hardening (priority 15) is the current milestone, and precedes automated GUI validation.** KF-BACKUP-003 Package D is merged via PR #76, KF-BACKUP-004 via PR #77, KF-BACKUP-005A via PR #79, KF-BACKUP-005B via PR #81, and **KF-BACKUP-005C (populated-target Active workflow convergence and conflict safety) via PR #83** (merge commit `bed54d01624e80ca6dd5adf8af097e64fe33e588`); `POST_MERGE_SYNC_ONLY` completed successfully for all. 005C evidence is **8/0/0** focused and **254/0/0** in each controlled Workers=1 and Workers=8 affected/regression run; final relevant reviews were **0 BLOCKER / 0 MAJOR / 0 MINOR**. The next documented residual is `LegacyReviewSummaries` ordering; the subsequent residual ordering remains unchanged. See [CURRENT_WORK.md](CURRENT_WORK.md) for lifecycle state.
...
14. **KF-BACKUP-005C:** Populated-target Active workflow convergence and conflict safety, including Active-vs-Active conflict handling, is merged and binding `master` behavior via PR #83 (merge commit `bed54d01624e80ca6dd5adf8af097e64fe33e588); `POST_MERGE_SYNC_ONLY` completed successfully. The existing Priority-15 residual ordering is next: `LegacyReviewSummaries` canonical ordering, `Learning.Cards`/Sense `StableId` canonical ordering, then the legacy v1 planner-label residual.
