...
| 15 | Portable merge integrity hardening | Current | Portable database export/import integrity is hardened in bounded packages before GUI automation begins. **KF-BACKUP-003 Package D, KF-BACKUP-004, KF-BACKUP-005A, KF-BACKUP-005B, KF-BACKUP-005C, and the `LegacyReviewSummaries` canonical ordering correction are committed and merged** via PRs #76, #77, #79, #81, #83, and #85. The `Learning.Cards`/Sense-`StableId` correction is implemented and bounded-validated on `fix/backup-v2-learning-cards-canonical-order-v1` (**119 passed / 0 failed / 0 skipped** final affected/regression scope), but is not yet merged or binding `master` behavior. Schema 10 and archive V2 remain unchanged. The remaining subsequent Priority-15 residual is the legacy v1 planner's analogous synthesized label; GUI automation remains after Priority 15. Candidate evidence is bounded automated unit/integration/contract evidence only; it is not ALL_AUTOMATED, ValidateAll, GitHub CI, platform/runtime, Release-build, rendered-GUI, device/emulator, APK/AAB, signing, publishing, or distribution evidence. |
...
**Portable merge integrity hardening (priority 15) is the current milestone, and precedes automated GUI validation.** KF-BACKUP-003 Package D is merged via PR #76, KF-BACKUP-004 via PR #77, KF-BACKUP-005A via PR #79, KF-BACKUP-005B via PR #81, KF-BACKUP-005C via PR #83, and **the `LegacyReviewSummaries` canonical ordering correction via PR #85** (feature head `baf5fcda0a017c1492a08dac730d683c1554784d`, merge commit `8eeaea58d87f9cfeb28cc4fc2520e5b277bb2526`); `POST_MERGE_SYNC_ONLY` completed successfully for all. The `Learning.Cards`/Sense-`StableId` correction is implemented and bounded-validated on `fix/backup-v2-learning-cards-canonical-order-v1`, with final affected/regression `TEST_ONLY` **119/0/0**, but is not yet merged and therefore is not binding `master` behavior. Its feature-branch publication/review lifecycle remains incomplete. The remaining subsequent residual is the legacy v1 planner analogous synthesized label. See [CURRENT_WORK.md](CURRENT_WORK.md) for lifecycle state.
...
14. **`LegacyReviewSummaries` canonical ordering correction:** Merged and binding `master` behavior via PR #85 (feature head `baf5fcda0a017c1492a08dac730d683c1554784d`, merge commit `8eeaea58d87f9cfeb28cc4fc2520e5b277bb2526`); `POST_MERGE_SYNC_ONLY` completed successfully. `BackupModelMapperV2` now orders V2 `LegacyReviewSummaries` by typed ascending content, explicitly distinguishing null from present UTC `DateTime.MinValue`; historical V1 mapper/writer behavior, Schema 10, archive V2, and merge policies are unchanged. Focused RED **1 failed / 0 passed** → GREEN **1 passed / 0 failed / 0 skipped**; independent review **0 BLOCKER / 0 MAJOR / 0 MINOR / 0 NIT**; bounded affected/regression TEST_ONLY **110/0/0**. Priority 15 remains Current. The remaining open residuals after this correction are: (1) `Learning.Cards`/Sense `StableId` cross-installation ordering; (2) the legacy v1 planner analogous synthesized-label residual.
15. **`Learning.Cards`/Sense-`StableId` canonical ordering correction:** Implemented and bounded-validated on `fix/backup-v2-learning-cards-canonical-order-v1`, but not yet merged or binding `master`. Final affected/regression `TEST_ONLY`: **119 passed / 0 failed / 0 skipped**. It corrects archive-local `c-*` semantic-binding instability without changing Schema 10, archive V2, DTOs, migrations/validators, V1 compatibility, merge/scheduler semantics, UI, transport, synchronization, public status/error codes, or persistence contracts. Priority 15 remains Current; the next residual is the legacy v1 planner analogous synthesized-label issue.
