...
The current product direction finishes real product functionality first: (1) Daily New-Word Limit & Learning-Day Infrastructure — Slice 1 is merged on `master` via PR #142 (Priority 19d below); German Enhanced Term Recognition Packages 1–5B are all merged on `master` (Priorities 17–19c below); Settings GUI & Learning-Day Defaults — Slice 2A is a reviewed, uncommitted candidate awaiting `DOCUMENT_ONLY`/`COMMIT_ONLY` (Priority 19e below) — not yet merged, (2) automated testing remains high priority, (3) user testing/feedback and UX cleanup follow once functionality is strong, (4) Public-Release Readiness / AAB creation (Priority 20) is **deferred** — not cancelled — and resumes only when the user explicitly authorizes/signals readiness to return to it.
...
| 19e | Settings GUI & Learning-Day Defaults (Slice 2A) | Current | Settings GUI exposing the Slice-1 learning-timezone (46-entry curated IANA catalog, dynamic DST-aware `(UTC±HH:mm) City` labels) and learning-day-cutoff (minute-precision `<input type="time">`) infrastructure; non-destructive "Restore default settings" action distinct from destructive "Reset all application data" with exact online-dictionary-consent preservation/revocation contracts; Enhanced Term Recognition missing-preference default changed from OFF to ON. No schema/archive-format change. Reviewed on branch `feature/settings-gui-learning-day-defaults-v1` (base `d3040b6a4aef0b9895b1aaad2cafc84315d70391`); independent `REVIEW_ONLY` 0 BLOCKER / 0 MAJOR, `REVIEW_APPROVED_FOR_DOCUMENT_ONLY`. **Not yet committed, pushed, opened as a pull request, or merged.** See [docs/CURRENT_WORK.md](CURRENT_WORK.md). |
