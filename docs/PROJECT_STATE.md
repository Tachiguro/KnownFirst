...
**Active development candidate — P16-A:** `feature/p16a-android-gui-foundation-v1` is an unmerged, non-binding Priority-16 candidate based on the binding `master` baseline above; exact live branch and PR state remain dynamic Git/GitHub facts. The first independent review found **0 BLOCKER / 2 MAJOR / 3 MINOR / 0 NIT**. A renewed independent static review found **0 BLOCKER / 2 MAJOR / 0 MINOR / 0 NIT** (`PR_CORRECTION_REQUIRED`): the late SDK-style intermediate-path assignment could overlap ordinary project-extension/NuGet state, and screenshot persistence and summary hashing used different captures. Both are corrected in `4545fcc927ffdd787c548b447cbd21d087460e25` (`fix: correct Android GUI test isolation and evidence`). Root `Directory.Build.props` establishes early, app-project-specific opt-in Debug GUI-test intermediate/project-extension isolation while ordinary KnownFirst Debug and KnownFirst.Core remain unaffected; the one native Release Notes capture is decoded once, persisted, and hashed from the same PNG bytes. Historical/superseded validation remains **159 passed / 0 failed / 0 skipped** MSTest and **5 passed / 0 failed / 0 cancelled / 0 skipped / 0 todo** Node. Current final bounded source-level validation is **160 passed / 0 failed / 0 skipped** MSTest and **7 passed / 0 failed / 0 cancelled / 0 skipped / 0 todo** Node, with pre/post `git diff --check` passed and seven correction-file hashes unchanged. Deterministic MSBuild property evaluation verified the GUI-test intermediate/project-extension root and output root while ordinary Debug/Core paths remain ordinary; it is source/build-configuration evidence only, not an Android build. PR #91 body reconciliation and this tracked-document reconciliation are complete; a renewed independent review of the final remote PR state remains pending before the user-only manual merge decision, followed only then by `POST_MERGE_SYNC_ONLY`. It is not merged production capability and establishes no Android platform build, package, installation, Appium/UiAutomator2/Chromedriver runtime, device/emulator, rendered interaction/screenshot, matrix, CI, or release evidence.
