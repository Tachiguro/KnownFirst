...
**Prioritization date:** 2026-08-09
...
| 15 | Portable merge integrity hardening | Current | Portable database export/import integrity is hardened in bounded packages before GUI automation begins. **KF-BACKUP-003 Package D is committed and merged via PR #76** (merge commit `17d3f1a031b9f319041ff1034a227d17b1029c4f`; `POST_MERGE_SYNC_ONLY` completed successfully), making the affected v2 export ordering total over emitted content. **KF-BACKUP-004 (Schema-9 `LearningReview` merge integrity) is committed and merged via PR #77** (merge commit `bec861fb8a054beb2804f1132b450da1e45dee90`; `POST_MERGE_SYNC_ONLY` completed successfully). **KF-BACKUP-005A (Schema-10 stable learning-workflow identity foundation) is committed and merged via PR #79** (merge commit `e56b8bfa27dfe1d630fbacfed24e6d56ea876026`; `POST_MERGE_SYNC_ONLY` completed successfully). KF-BACKUP-005B (portable Active workflow export/restore into empty target) is the next bounded development package, followed by KF-BACKUP-005C (populated-target Active workflow convergence). The remaining portable-integrity residuals (`LegacyReviewSummaries` ordering, mid-session review-event export policy, `Learning.Cards`/Sense `StableId` cross-installation ordering, and the legacy v1 planner's analogous synthesized label) are assessed in further bounded packages before GUI automation. Evidence is automated only — no rendered-GUI, runtime, platform, Release-build, device, or AAB evidence. |
...
13. **KF-BACKUP-005B (planned):** Portable Active learning-workflow export and restore into an empty installation from the last durably committed application state.
14. **KF-BACKUP-005C (planned):** Populated-target Active workflow convergence and conflict safety.
