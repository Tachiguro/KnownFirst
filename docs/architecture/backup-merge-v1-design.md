...
**Status:** Proposed design, revision 2. Slices 1 (pure merge contracts) and 2 (validated safety copies) are implemented and merged to `master`. Slice 3 (read-only merge preflight, §16/§17) is implemented only on the still-unmerged `feature/backup-merge-preflight-v1` branch. No schema migration, merge writer, Import routing, or UI change has been made — those remain later slices, with KF-MEANING-001 (§16, [BACKLOG.md](../BACKLOG.md)) required before the merge writer (slice 5).
...
## 16. Approved meaning-centric model — Word → SemanticMeaning → AnswerVariant (KF-MEANING-001)

This section records the product model KF-MEANING-001 will implement and the identity/merge-rule revisions KF-BACKUP-002 Slice 3 already made to support it in its read-only preflight, without implementing any of KF-MEANING-001 itself (no schema migration, no scheduler, no writer — see §12 slice list; KF-MEANING-001 is its own backlog item, gated to land before slice 5's merge writer).
