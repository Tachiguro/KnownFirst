...
| 15 | Portable merge integrity hardening | Current | Portable database export/import integrity is hardened in bounded packages before GUI automation begins. **KF-BACKUP-003 Package D, KF-BACKUP-004, and KF-BACKUP-005A are committed and merged** via PRs #76, #77, and #79. **KF-BACKUP-005B is the current feature-branch package:** Schema-10 Active learning-workflow export and empty-target restore are implemented locally, final focused `TEST_ONLY` is green (**135 passed / 0 failed / 0 skipped**), and documentation reconciliation is complete; the package remains unmerged until its normal repository lifecycle completes. Schema 10 and archive V2 remain unchanged. **KF-BACKUP-005C is the next bounded implementation package after 005B completes its lifecycle** and still owns populated-target Active convergence and conflict safety. All remaining portable-integrity residuals (`LegacyReviewSummaries` ordering, `Learning.Cards`/Sense `StableId` cross-installation ordering, and the legacy v1 planner's analogous synthesized label) remain open before GUI automation. Evidence for 005B is automated `TEST_ONLY` only — no `ValidateAll`, rendered GUI, platform build/runtime, Release build, device/emulator, APK/AAB, signing, publishing, or distribution evidence. |
...
**Portable merge integrity hardening (priority 15) is the current milestone, and precedes automated GUI validation.** KF-BACKUP-003 Package D is merged via PR #76 (merge commit `17d3f1a031b9f319041ff1034a227d17b1029c4f`), KF-BACKUP-004 (Schema-9 `LearningReview` merge integrity) is merged via PR #77 (merge commit `bec861fb8a054beb2804f1132b450da1e45dee90`), and KF-BACKUP-005A (Schema-10 stable learning-workflow identity foundation) is merged via PR #79 (merge commit `e56b8bfa27dfe1d630fbacfed24e6d56ea876026`), with `POST_MERGE_SYNC_ONLY` completed successfully for all. **KF-BACKUP-005B (portable Active workflow export and restore into an empty target) is implemented locally and focused final `TEST_ONLY` is complete on the feature branch; documentation reconciliation is complete and the package remains unmerged until its normal repository lifecycle completes.** KF-BACKUP-005C (populated-target Active workflow convergence) is the next bounded implementation package after the 005B lifecycle. Remaining portable-integrity residuals stay open before GUI automation begins. See [CURRENT_WORK.md](CURRENT_WORK.md) for the active package state.
...
13. **KF-BACKUP-005B (current feature-branch package):** Implementation, final focused `TEST_ONLY`, and documentation reconciliation are complete locally; ordinary Schema-10 export carries the Active workflow's durable queue and committed review history, and empty-target restore resumes it from the last committed application/database state. Completed workflow regression coverage remains green. Schema-8/9 stay Completed-only. Populated-target Active archives remain fail-closed. The package remains unmerged until its normal repository lifecycle completes; KF-BACKUP-005C follows after that lifecycle.
14. **KF-BACKUP-005C (planned next after 005B lifecycle completion):** Populated-target Active workflow convergence and conflict safety, including Active-vs-Active conflict handling. Not implemented by 005B.
