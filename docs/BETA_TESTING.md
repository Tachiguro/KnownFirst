...
16. Settings → Help and Support contains the functional "Report a bug" action. Verify on the physical device that activating it opens the system email composer with the exact recipient (`Tachiguro+KnownFirst_BugReport@gmail.com`), localized subject, and structured template prompts without automatically sending, and that if no mail app is available the fallback panel allows copying the exact address. Confirm that `Support KnownFirst` and any "coming soon" placeholder panel remain completely absent. Verify this on the actual device under test and record the concrete result; source/contract test evidence is not physical-device evidence. The Help and Support heading and the build-identity line remain present and are expected.
