...
The current product direction finishes real product functionality first: (1) Daily New-Word Limit & Learning-Day Infrastructure — Slice 1 is currently active on branch `feature/daily-new-word-limit-learning-day-v1` (Priority 19d below; implementation and independent review complete, undergoing documentation reconciliation); German Enhanced Term Recognition Packages 1–5B are all merged on `master` (Priorities 17–19c below), (2) automated testing remains high priority, (3) user testing/feedback and UX cleanup follow once functionality is strong, (4) Public-Release Readiness / AAB creation (Priority 20) is **deferred** — not cancelled — and resumes only when the user explicitly authorizes/signals readiness to return to it.
...
| 19d | Daily New-Word Limit & Learning-Day Infrastructure (Slice 1) | Current (candidate on branch) | Non-visual daily new-word budget ($N \in \{5, 10, 20, 30, 50\}$, default 10), durable `ActiveBudgetDay` and `Bridge` state, timezone and cutoff infrastructure, Schema-12 persistence, and active-session rollover reconciliation on branch `feature/daily-new-word-limit-learning-day-v1`. Implementation and independent review complete; undergoing documentation reconciliation. |
