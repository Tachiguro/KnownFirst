...
| 15 | Portable merge integrity hardening | Current | Earlier packages through PR #87 are merged; PR #88 completed the PR #87 documentation closure and `POST_MERGE_SYNC_ONLY` at current master `133d34366204979d2905c665370531547a7a0b98`. The active final candidate is the Occurrence action-key correction on `fix/backup-v1-planner-action-key-v1`: implemented, bounded-validated, committed at `edbb49a87ff3f37337c413111a60f6cfa6805b88`, and pushed; PR, independent-review, and merge lifecycle are pending. It is not merged or binding master behavior. It replaces the production-representable `SentenceId:VocabularyId` lookup collision with `SourceMaterialArchiveId:Occurrence.Order`, leaving semantic identity and compatibility contracts unchanged. Focused TDD **0/2/0 → 2/0/0**; bounded TEST_ONLY **257/0/0**. Evidence is isolated synthetic-SQLite automated component/integration/contract evidence only, not ALL_AUTOMATED, ValidateAll, GitHub CI, platform/runtime, GUI/device, package, or release evidence. |
...
**Portable merge integrity hardening (priority 15) is the current milestone, and precedes automated GUI validation.** Earlier packages through PR #87 are merged; PR #88 completed the PR #87 documentation closure (feature head `4fc8b15f0861ab264ef7518ef66e810b0cf3c15c`, merge commit `133d34366204979d2905c665370531547a7a0b98`) and `POST_MERGE_SYNC_ONLY`. The active final candidate is the Occurrence action-key correction: it is implemented, bounded-validated (**RED 0/2/0 → GREEN 2/0/0; TEST_ONLY 257/0/0**), committed at `edbb49a87ff3f37337c413111a60f6cfa6805b88`, and pushed on `fix/backup-v1-planner-action-key-v1`; PR, independent-review, and merge lifecycle are pending. It is not merged or binding master behavior. The legacy LearningReview label is not a production-writer defect on the current V1-to-V2 route. See [CURRENT_WORK.md](CURRENT_WORK.md) for lifecycle state.
...
16. **Occurrence action-key correction:** active final Priority-15 candidate on `fix/backup-v1-planner-action-key-v1`, implemented and bounded-validated, committed at `edbb49a87ff3f37337c413111a60f6cfa6805b88`, and pushed; PR, independent-review, and merge lifecycle are pending. It is not merged or binding master behavior. It uses `SourceMaterialArchiveId:Occurrence.Order` for action lookup identity rather than the colliding `SentenceId:VocabularyId`; semantic occurrence identity and the listed compatibility contracts are unchanged. Focused TDD **0/2/0 → 2/0/0**; bounded TEST_ONLY **257/0/0**. Priority 15 remains Current; Priority 16 remains Planned.
