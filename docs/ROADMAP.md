...
The current active product direction is the **First-Run Onboarding + Daily New-Word Budget UX** program, starting with Slice 1 (install-origin classification, existing-install grandfathering, legacy budget pinning, and reset contracts) on branch `feature/onboarding-install-origin-foundation-v1` (Current). Subsequent slices will implement visible onboarding UI, Display Name, new daily-budget presets (1 / 5 Recommended / 10 / Custom), default change from 10 to 5, and Home personalization (Planned). Broad feature expansion freezes after the onboarding program except bug fixes, required release corrections, and evidenced tester feedback. Security, test-effectiveness, governance, repository hardening, public-trust/OSS, and distribution/branding/website work remain later priorities. Beta-13 Build-14 AAB creation and packaging evidence are recorded as completed in repository history (PR #152); Google Play Internal Testing distribution and Public-Release Readiness (Priority 20) remain separately deferred until explicitly authorized.
...
| 19f | First-Run Onboarding & Daily-Budget UX — Slice 1 (Install-Origin Foundation & Reset Contracts) | Current | Application-local preference-backed onboarding state (`Required`/`InProgress`/`Completed`), startup install-origin classification via legacy preference evidence without database-file dependency, grandfathered legacy budget pinning (10), destructive/non-destructive reset contracts (`Required` on full reset, unchanged on Restore Defaults). No schema/archive change (`DatabaseSchema.CurrentVersion` = 12, archive V2). Active on `feature/onboarding-install-origin-foundation-v1`. |
| 19g | First-Run Onboarding & Daily-Budget UX — Later Slices (Visible Onboarding, Display Name, Daily Budget UX, Home Personalization) | Planned | First-run onboarding step UI, routing/gating in `Routes.razor`, Display Name preference & Home greeting, daily-budget presets `1 / 5 Recommended / 10 / Custom`, default change from 10 to 5, >15 custom budget warning, What's New integration, GUI test profile suppression. |
